...
function Leaflet(params){
    ...
    var map = L.map(containerId).setView([lat,lng], 2);
...
var EditorMap = function(params) {
    ...
    map.onInit(function(){
        ...
        editorLayer = EditorLayer({
            ...
            .onEndpointEvent('mouseenter',function(d){
                lmap.dragging.disable();
            })
            .onEndpointEvent('mouseout' ,function(d){
                lmap.dragging.enable();
            })
            .onEndpointEvent('click', function(d){
                if(d.event.altKey){
                    removeWaypoint(d);
                }
            })
    ...
    if (params.topology) {
        map.init();
        let json_obj; 
        if(JSON.parse(params.topology)){
            json_obj = JSON.parse(params.topology);
            let topo = json_obj.results[0];
            if(!topo){
                console.log('Map topology doesnt exist');
                return;
            }
            map.topology(topo);
            map.cacheTopology();
